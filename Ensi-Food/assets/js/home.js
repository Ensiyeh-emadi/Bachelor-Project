...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Function to switch between tabs and update accessibility attributes//جابجایی تب ها
  ...
  // Function to handle search input and redirect to recipes.html//هندل کردن سرچ
  ...
  // Event listener to hide error message on search field input//پیغام قرمز سرچ
  ...
  // Event listener to handle search when Enter key is pressed//سرچ با اینتر 
  searchField.addEventListener("keydown", (e) => {
    if (e.key === "Enter") handleSearch();
  });

  // Defines several functions and event listeners for handling tab switching, search functionality, and page-related events.//اینپوت سرچ و خالی میکنه
  ...
  //  Iterates over each tab button, adding an event listener to switch tabs and call addTabContent when a new tab is clicked.//لودینگ تب 
  ...
  // Initializes an array of cuisine types and selects all DOM elements with the attribute data-slider-section.//ساخت بنر 
  let cuisineType = ["Asian", "French"];

  const sliderSections = document.querySelectorAll("[data-slider-section]");

  //  Iterates over each slider section to populate it with recipe data.//ساخت کارت اسلایدر 
  for (const [index, sliderSection] of sliderSections.entries()) {
    ...
    // Selects the slider wrapper element within the current slider section.//تگ والد اسلایدر 
    ...
    fetchData(
      ...
      function (data) {
        ...
        // Maps through each item in the fetched data to create and append slider card elements.//ساخت کارت اسلایدر
        data.hits.map((item) => {
          const {
            recipe: { image, label: title, totalTime: cookingTime, uri },
          } = item;
