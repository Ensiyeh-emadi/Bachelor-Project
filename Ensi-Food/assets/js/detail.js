...
//انتخاب کردن تگ والد صفحه جزییات
...
//داده هایی که نیاز داریم و از api جدا میکنه
fetchData(null, (data) => {
  ...
  // Sets the document title, for the browser tab, using the recipe title and an identifier ("Ensi Food").
  document.title = `${title} - Ensi Food`;
  ...
  const isSaved = window.localStorage.getItem(`cookie-recipe${recipeId}`);//سیو شدن غذا چک میکنه

  // Maps over the tags array to generate HTML elements for each tag, categorizing them by type (cuisineType, diet, or dishType) and creating hyperlinks.//تگ ها  رو درست میکنه
  tags.map((tag) => {
    let type = "";
  ...
  // Maps over the ingredients array to generate HTML elements for each ingredient, creating a list of ingredients.//لیست مواد تشکیل دهنده
  ingredientLines.map((ingredient) => {
    ingredientItems += `<li class="ingr-item">${ingredient}</li>`;
  });

  // Dynamically generates HTML structure using template literals and injects it into the detailContainer element, rendering the recipe details on the web page.//اجزای صفحه لود میکنه به صورت داینامیک
