...
// Adds a click event listener to filter togglers, toggling the visibility of the filter bar and overlay.//فیلتر با کلیک
...
// Adds a keydown event listener to the filter search input, handling the submission of filter queries when the Enter key is pressed.//اینتر سرچ
...
// Determines whether to display the filter count based on the presence of filter queries.//تعداد فیلتر برای نسخه موبایل
...
// Applies filter queries to the filter bar if they exist.//بررسی برای بودن اطلاعات
queryStr &&
...
// Adds a scroll event listener to show/hide the filter button based on the user's scroll position.//اسکرول لودینگ
...
// Initializes the grid list with loading skeleton cards and fetches recipes based on filter queries or default queries using the fetchData function.//ساخت اسکلتون
